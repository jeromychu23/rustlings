...
impl TryFrom<RawCycleRow> for CycleRow {
    ...
    fn try_from(raw: RawCycleRow) -> Result<Self, Self::Error> {
        let tail_number = raw.tail_number.trim().to_ascii_uppercase();
        if tail_number.is_empty() {
            return Err(CycleRowError::EmptyTail {
                row_number: raw.row_number,
            });
        }

        if !(0..=u16::MAX as i32).contains(&raw.cycles) {
            return Err(CycleRowError::CyclesOutOfRange {
                row_number: raw.row_number,
                value: raw.cycles,
            });
        }
