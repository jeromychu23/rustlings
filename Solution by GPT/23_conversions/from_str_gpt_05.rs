...
impl FromStr for MaintenanceRow {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        if fields.len() != 3 {
            return Err(MaintenanceRowParseError::BadLen);
        }

        let tail_number = fields[0].trim();
        if tail_number.is_empty() {
            return Err(MaintenanceRowParseError::MissingTail);
        }

        let ata_code = fields[1].trim();
        if ata_code.is_empty() {
            return Err(MaintenanceRowParseError::MissingAta);
        }

        let flight_hours = fields[2]
            .trim()
            .parse::<u32>()
            .map_err(MaintenanceRowParseError::ParseHours)?;

        Ok(Self {
            tail_number: tail_number.to_ascii_uppercase(),
            ata_code: ata_code.to_string(),
            flight_hours,
        })
    }
}
