...
//
// Task: Convert each flight-hour value into a label such as "120 FH".
// Use `map()` and `collect()`. Do not use loops or manual index traversal.

fn format_flight_hours(hours: &[u32]) -> Vec<String> {
    // TODO: Transform every value while preserving input order.
    Vec::new()
}
